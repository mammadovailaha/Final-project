...
const FaqItem: React.FC<FaqItemProps> = ({ id, question, answer, isOpen, onToggle }) => {
  return (
    <div
      className={`w-88 md:min-w-3xl p-5 flex flex-col items-center bg-[#c8edf8] rounded-2xl md:rounded-3xl ${
        isOpen ? "h-auto min-h-35 justify-start" : "h-12 md:h-18 justify-center"
      }`}
    >
      <div
        onClick={() => onToggle(id)}
        className={`w-[95%] flex justify-center items-center rounded-3xl ${
