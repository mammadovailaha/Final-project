...
const  data= staffData.find((item) => item.id === Number(id));
  ...
  return (
    <div style={{padding:"20px"}} className="w-full h-screen flex flex-col md:flex-row justify-center items-center gap-10 bg-[#F7FCFF] ">
      ...
      <h1 className="text-2xl md:text-4xl font-bold text-[averta]">{data?.fullName}</h1>
      <h3 className="text-[averta]">{data?.role}</h3>
      <p className="text-[averta] text-center">{data?.staffInfo}</p>
      </div>
    </div>
  )
}
